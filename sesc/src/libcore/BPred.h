...
public:
    ...
    PredType predict(const Instruction *inst, InstID oracleID, bool doUpdate) {
        ...
        // Choosing to try and count statics here due to troubles with miscounts
        // Open the log file if not already open
        if (!branchLogFile.is_open()) {
            branchLogFile.open("branch_log.txt", std::ios::out | std::ios::trunc);
            if (!branchLogFile.is_open()) {
                std::cerr << "Failed to open branch_log.txt for writing" << std::endl;
                exit(1);
            }
            // Write header
            branchLogFile << "BranchID,CorrectPrediction,BranchType\n";
        }

        uint64_t branchID = inst->currentID(); // Use getAddr() if this doesn't work
        ...
        if (inst->isFuncRet()) {
            branch_type = "Return";
        } else if (inst->isBJCond()) {
            branch_type = "BJCond";
        } else if (inst->isBranchTaken()) {
            branch_type = "Unconditional";
        } else if (inst->isFuncCall()) {
            branch_type = "FuncCall";
        } else {
            branch_type = "Other";
        }
